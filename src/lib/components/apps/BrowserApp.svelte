<script lang="ts">
  ...
  function addTab(initialTargetUrl?: string) {
    ...
    const targetUrl = initialTargetUrl ? (initialTargetUrl.includes('/scramjet/') ? initialTargetUrl : `${PROXY_HOST}/scramjet/${encodeURIComponent(initialTargetUrl)}`) : ''
  ...
  function cleanDisplayUrl(url: string): string {
    if (!url) return ''
    try {
      if (url.includes('?u=') || url.includes('&u=') || url.includes('?url=') || url.includes('&url=')) {
        const parsed = new URL(url, PROXY_HOST)
  ...
  function buildProxyUrl(rawInput: string): { iframeSrc: string; display: string } {
    ...
    if (!query) {
      return {
        iframeSrc: `${PROXY_HOST}/?transport=${selectedTransport}`,
        display: '',
      }
    }
  ...
  function handleWindowMessage(e: MessageEvent) {
    ...
    if (incomingUrl && incomingUrl.trim()) {
      ...
      const isProxyHome = clean.includes(PROXY_HOST) || rawUrl.includes(PROXY_HOST) || clean === 'about:blank'
...
<div class="browser-window theme-{impersonationTheme}" on:click={closeAllMenus}>
  ...
  <div class="browser-body">
    {#each tabs as tab (tab.id)}
      {#if !tab.url}
        <div class="start-page" class:active={tab.id === activeTabId}>
